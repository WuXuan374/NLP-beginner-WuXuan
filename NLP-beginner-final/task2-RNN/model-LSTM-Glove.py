...
def words_encode(sentences, WORD_INDEX, vocab_size, embedding_dim, weights_matrix):
    '''

    :param sentences:
    :param WORD_INDEX:  ('film':0)
    :param vocab_size: 每句话的长度
    :return: word_vector : 将sentences转化为glove pre_trained embeddings
    不使用自带的embedding层了， 直接把sentences 表示为embedding
    '''
    word_vector = np.zeros((len(sentences), vocab_size, embedding_dim), dtype=np.float)
    i = 0
    for sentence in sentences:
        j = 0
        vector = np.zeros((1, vocab_size, embedding_dim), dtype=np.float)
        for word in sentence:
            if word in WORD_INDEX.keys():  # 如果该词位于词典中
                vector[0, j, :] = weights_matrix[WORD_INDEX[word]]
            else:
                # 没有在词典中出现的词，统一用0表示（词典里头1-> vocab_size)
                vector[0, j, :] = weights_matrix[0]
            j = j+1
...
# load vectors
with open('glove_wordvec/words200d.pickle', 'rb') as handle:
    words = pickle.load(handle)
with open('glove_wordvec/Word_Index200d.pickle', 'rb') as handle:
    word2idx = pickle.load(handle)
vectors = np.load('glove_wordvec/WordVectors200d.npy')
...
# embedding_layer = create_embedding_layer(weights_matrix, WORD_INDEX)


# 文本使用glove embedding
train_sentences = words_encode(train_sentences, WORD_INDEX, vocab_size, embedding_dim, weights_matrix)
val_sentences = words_encode(val_sentences, WORD_INDEX, vocab_size, embedding_dim, weights_matrix)
test_sentences = words_encode(test_sentences, WORD_INDEX, vocab_size, embedding_dim, weights_matrix)
# 对标签进行编码 变为numpy 数组
# test_labels不用编码
...
config = Config(vocab_size, model_path="model-BidirectionLSTM_glove_200d.h5")
# 建立模型
...
input_shape = train_sentences.shape
print(input_shape)
...
callback1 = keras.callbacks.EarlyStopping(monitor='val_loss', patience=4, restore_best_weights=True)
